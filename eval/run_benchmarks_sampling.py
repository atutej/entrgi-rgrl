...
def _parse_args():
    ...
    # sharding, for splitting one benchmark's data across concurrent multi-GPU processes
    # (see run_sharded.py). Dataset-agnostic: applied to the already-loaded `data` list, after
    # any --test_sample_size subsampling, regardless of which benchmark it came from.
    ...
    parser.add_argument("--diffusion_batch_size", type=int, default=None, help="Batch size for dream/llada sampling backends (default: 8, see llm_utils._batch_diffusion_query).")
    ...
    if args.model_saving_name is None:
        # Include the checkpoint's PARENT directory name, not just its own basename --
        # "checkpoint-500" alone collides across every run that happens to save a checkpoint at
        # the same step (confirmed: this is exactly why old cache files were named
        # "dream-checkpoint-500_*.sqlite" instead of something including the actual run name).
        # The parent directory is each training run's own output dir (e.g.
        # "dream-grpo-wildchat-1019400"), which already contains the SLURM job ID and is unique
        # per run by construction -- so this is enough to disambiguate without hardcoding any
        # specific ID.
        def _basename_with_parent(path):
            path = path.rstrip("/")
            parent = os.path.basename(os.path.dirname(path))
            leaf = os.path.basename(path)
            return f"{parent}-{leaf}" if parent else leaf

        if args.model.startswith(("vllm/", "dream/", "llada/")):
            prefix, path = args.model.split("/", 1)
            model_base_name = prefix + "/" + _basename_with_parent(path)
        else:
            model_base_name = _basename_with_parent(args.model)
...
def output_filename_func(args):
    ...
    shard_suffix = f"_shard{args.shard_index}of{args.num_shards}" if args.num_shards > 1 else ""
    output_filename = f"{args.benchmark}_max{args.max_tokens}t{args.temperature}p{args.top_p}n{args.n}_{args.seed}{shard_suffix}.json"
...
def eval_benchmark(args):
    ...
    shard_suffix = f"_shard{args.shard_index}of{args.num_shards}" if args.num_shards > 1 else ""
    cache_file = os.path.join("caches", f"{args.model_saving_name}_{args.benchmark}{shard_suffix}.sqlite")
    ...
    if os.path.exists(checkpoint_filename) and not args.force_overwrite:
        ...
        outputs = cached_batch_query(
            ...
            aux_kwargs={
                ...
                **({"diffusion_batch_size": args.diffusion_batch_size} if args.diffusion_batch_size is not None else {}),
            },
