...
class TrainingArguments(RGRLGDPOConfig):
    ...
    loss_backend: str = field(
        ...
        metadata={
            ...
            "single-random-masking-draw log-prob estimator -- the pre-existing behavior, kept "
            "reachable but no longer the default)."
        },
    )
...
def train():
    ...
    logger.info(f"Starting RG-RL online SFT (Dream), loss_backend={training_args.loss_backend}...")
    trainer = trainer_cls(
        model=model,
