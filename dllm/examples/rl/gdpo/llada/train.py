"""
GDPO training for LLaDA with diffusion denoising.

Same as `examples/rl/grpo/llada/train.py`, except the loss/log-likelihood is GDPO's own
faithful port (`dllm.pipelines.rl.gdpo.DiffuGRPOTrainerWithGDPOEstimator`) instead of TRL's
...
- 1 GPU, quick test (no LoRA):
    accelerate launch \
        ...
        examples/rl/gdpo/llada/train.py \
        --model_name_or_path GSAI-ML/LLaDA-8B-Instruct \
        --load_in_4bit True \
        --dataset gsm8k --max_steps 50 \
        --output_dir .models/LLaDA-8B-Instruct/gdpo
...
- 1 Node, 8 GPUs:
    sbatch --gres=gpu:8 scripts/train.slurm.sh \
        ...
        --script_path "examples/rl/gdpo/llada/train.py" \
        -- --dataset gsm8k --output_dir .models/LLaDA-8B-Instruct/gdpo
"""
...
from dllm.pipelines.rl.gdpo import GDPOEstimatorConfig, DiffuGRPOTrainerWithGDPOEstimator
...
class TrainingArguments(GDPOEstimatorConfig):
    output_dir: str = ".models/LLaDA-8B-Instruct/gdpo"
    dataset: Optional[str] = field(
        default="gsm8k",
        metadata={"help": "Dataset to train on: gsm8k, countdown, sudoku, math, code, wildchat, magpie, lmsys."},
    )
    ...
    reward_model: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Reward model for scoring completions (wildchat, magpie, lmsys). "
...
def train():
    ...
    # ---- Trainer ----------------------------------------------------------------
    logger.info("Start GDPO training...")
    trainer = DiffuGRPOTrainerWithGDPOEstimator(
        model=model,
